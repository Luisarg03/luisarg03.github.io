...
Make the htop hero skill rows (001-008) expandable in htop tree-view style: clicking or pressing Enter/Space on a row reveals the category's real `skills[]` array from cv.ts as indented sub-rows, with chevron indicators, keyboard accessibility, and no-JS static fallback.
