...
Add optional quantified proof fields to the existing content surfaces — `evidence` on skill categories and `scaleMetric` on project frontmatter — rendered only when present, so the site gains real business metrics without changing anything when fields are absent.
