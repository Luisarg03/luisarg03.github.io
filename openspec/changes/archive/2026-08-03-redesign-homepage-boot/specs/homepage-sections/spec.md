...
The homepage (`/`) SHALL render as a single scrollable page under `BaseLayout` composed of: the boot screen module (viewport 1), then the identity (whoami), skills (htop), experience (journalctl), and contact modules in vertical order, per the `boot-into-content` spec.
...
- **THEN** the boot screen module renders as the first viewport
- **AND** scrolling reveals the whoami, htop, journalctl, and contact modules in order
...
**Reason**: Replaced by the whoami identity module; the neofetch card no longer fits the boot-into-content layout.
**Migration**: Identity facts now render in the whoami module per the `home-hero` delta spec.

### Requirement: Skills marquee strip
**Reason**: Replaced by the htop skills module; the first-screen marquee is superseded by the boot → whoami → htop module flow.
**Migration**: Skill category chips now render inside the htop module per the `skills-visualization` delta spec.
