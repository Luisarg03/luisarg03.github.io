...
Replace the first viewport of the site with a real htop window that maps every row to real site data: the identity as root process PID 000 and the 8 skill categories from cv.ts as child processes, with a single orchestrated handoff animation from the boot sequence, a condensed mobile layout, and palette anchors re-pointed to the window.
