...
The `/now` page SHALL render travel entries as an expandable, timestamped log in the existing style of the site's journalctl/log UI (e.g. `[2024-04-10] [Japan] kyoto-temples: ...`), sorted by date descending. Each entry SHALL be collapsed by default, showing only the timestamp, location, and caption line.

#### Scenario: Log lists entries newest first
- **WHEN** the travel log renders on `/now`
