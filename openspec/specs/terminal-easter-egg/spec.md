# Terminal Easter Egg Specification

## Purpose

Define the interactive terminal easter egg: the full terminal emulator (`Shell.astro`) rendered as a standalone page at `/terminal` with a minimal layout (no `BaseLayout` header/footer) to preserve the immersive feel. The terminal is reachable from any page via the `Ctrl+Shift+T` keyboard shortcut and from the command palette, provides back navigation to the homepage, and announces the shortcut via a footer hint.
